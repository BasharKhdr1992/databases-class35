...
connection.query('select Name from city where CountryCode = (select Code from country where Name = ?);',['Netherlands'], (err, result) => {
  if (err) console.log(err);
  else {
    for (var i = 0; i < result.length; i++) {
      console.log(result[i].Name);
...
connection.query('select sum(population) as  world_population from country;',(err, result) => {
  if (err) console.log(err);
  else {
